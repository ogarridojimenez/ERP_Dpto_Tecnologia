...
async function main() {
  ...
  console.log("Tipos:", Object.keys(tipoMap).join(", "));

  for (const room of DEFAULT_ROOMS) {
    const { data: existing } = await supabase
      .from("locales").select("id").eq("codigo", room.id).maybeSingle();
    let localeId;
    ...
    if (existing) {
      ...
      const { data: loc, error } = await supabase
        .from("locales").insert({
          organization_id: ORG_ID, codigo: room.id, nombre: room.id,
          tipo: room.type, estado: "activo",
          observaciones: room.condition || null, user_id: ADMIN_ID,
        }).select("id").single();
      if (error) { console.error(`ERR locale ${room.id}:`, error.message); continue; }
      localeId = loc.id;
      console.log(`+ locale: ${room.id}`);
    }
    ...
    for (const eq of room.equipment) {
      ...
      const { data: existingMedio } = await supabase
        .from("medios").select("id").eq("codigo", codigo).maybeSingle();

      if (!existingMedio) {
        const { error } = await supabase.from("medios").insert({
          organization_id: ORG_ID, locale_id: localeId, tipo_medio_id: tipoId,
          codigo, nombre: `${tipoNombre} ${room.id}`,
          numero_medio_basico: codigo,
          estado: "bueno", user_id: ADMIN_ID,
        });
        if (error) console.error(`  ERR medio ${codigo}:`, error.message);
      }
    }
  }

  const { data: locCount } = await supabase.from("locales").select("id", { count: "exact", head: true });
  const { data: medCount } = await supabase.from("medios").select("id", { count: "exact", head: true });
  console.log(`\nDone: ${locCount?.length || "?"} locales, ${medCount?.length || "?"} medios`);
}
